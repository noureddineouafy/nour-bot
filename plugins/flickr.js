...
let handler = async (m, { conn, isOwner, usedPrefix, command, args }) => {
	let query = "هذا الامر خاص بتحميل الصور بجودة ممتازة من موقع  flickr نكتب هكذا \n\n*.flickr* man"
	let text
	if (args.length >= 1) {
	...
	m.reply(wait)
	 ...
	 conn.sendFile(m.chat, resul, "هذه هي الصورة التي طلبتها", "Result Flickr: *" + text.toUpperCase() + "*", m)
  } catch (e) {
  throw eror
 }
}
